...
1. **Update Version Numbers**
   ...
   # (No CHANGELOG to maintain by hand: release.yml auto-generates the
   #  changelog from git commit messages when the release tag is pushed.)
   ```
...
- [ ] Release notes will be auto-generated by `release.yml` from commit messages since the previous tag (no manual CHANGELOG.md required)
